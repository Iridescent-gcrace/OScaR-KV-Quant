...
mha_fwd_kvcache(at::Tensor &q,                       // batch_size x seqlen_q x num_heads x head_size
                ...
                int num_splits,
                c10::optional<const at::Tensor> &k_norm_,
                c10::optional<const at::Tensor> &k_norm_new_
                ) {
    ...
    if (k_norm_.has_value()) {
        ...
        TORCH_CHECK(k_norm.dtype() == q_dtype, "Packed key norm must have the same dtype as query");
    ...
    if (k_.has_value()) {
        ...
        if (k_norm_new_.has_value()) {
            ...
            TORCH_CHECK(k_norm_new.dtype() == q_dtype, "Residual key norm must have the same dtype as query");
            TORCH_CHECK(k_norm_new.dim() == 2, "Residual key norm must have shape [batch, residual_block_size]");
...
at::Tensor kvcache_qpack_preprocess_k(
    ...
    TORCH_CHECK(k.size(0) == v.size(0) && k.size(1) == v.size(1) && k.size(2) == v.size(2) && k.size(3) == v.size(3),
    ...
    const auto batch_size = k_packed_input.size(0);
    const auto seqlen_k = k_packed_input.size(1);
    const auto num_heads = k_packed_input.size(2);
    const auto head_dim = k_packed_input.size(3);

    auto k_unpad = k_packed_input.reshape({batch_size * seqlen_k, num_heads, head_dim});
    auto v_unpad = v_contig.reshape({batch_size * seqlen_k, num_heads, head_dim});
