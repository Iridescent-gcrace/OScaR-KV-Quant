...
__global__ void preprocess_k_cache_kernel(
    const scalar_t* __restrict__ input,
    scalar_t* __restrict__ output,
    scalar_t* __restrict__ norm_output,
    int total_tokens,
    int num_heads,
    int head_dim,
    bool apply_hadamard,
    ...
    // Interim implementation: move K-side Hadamard/norm off the Python hot path first.
    // This is a shared-memory FWHT for head_dim=128, not the final HadaCore-style tensor-core path.
    ...
    if (tid == 0 && norm_output != nullptr) {
        norm_output[token_idx] = from_float_device<scalar_t>(token_norm);
    }
...
void launch_preprocess_k_cache(
    ...
    scalar_t* norm_ptr = apply_norm ? reinterpret_cast<scalar_t*>(key_norm_out.data_ptr()) : nullptr;

    preprocess_k_cache_kernel<scalar_t><<<total_tokens, kThreads, kThreads * sizeof(float), at::cuda::getCurrentCUDAStream()>>>(
        input_ptr,
        output_ptr,
        norm_ptr,
        total_tokens,
...
std::tuple<at::Tensor, at::Tensor> preprocess_k_cache_cuda(
    ...
    auto key_norm_out = apply_norm
        ? torch::empty({key_states.size(0), key_states.size(1)}, key_states.options())
        : torch::empty({0}, key_states.options());
