...
- For the current stable `2-bit + k-channel` path, `residual_block_size=256` is the user-visible flush threshold and the CUDA residual tile size.
...
- Token-wise K norm is stored separately as fp32 metadata `k_norm`.
...
- The current `2-bit + k-channel + residual_block_size=256` path flushes directly with the CUDA kernel.
...
- [x] Align `2-bit + k-channel` pack / residual flush on `256`-token blocks
...
- Extension builds successfully with:
  - `.venv-local/bin/python setup.py install`
- `evaluation/example.py` runs successfully for:
  - `Qwen3-8B`
  - `bit_decoding`
  - `2-bit`
  - `KV_ROTATION=hadamard`
  - `KV_NORM=1`
- `compute-sanitizer --tool memcheck` reports `ERROR SUMMARY: 0 errors` on a packed+residual HN decode smoke case.
- `evaluation/scripts/run_qwen3_gsm8k_accuracy.py --variant hada_norm --indices 0-9` reports `8/10`, matching the current `plain` and `fa2` small-slice accuracy under the same prompt and parser.
